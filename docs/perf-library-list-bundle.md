...
- **CI automation** (commit `d0cbd0a`, hardened in the final follow-up): a `seed-content` job in `.github/workflows/ci.yml` runs the seed script automatically on every push to `main`. This closes a real gap: a source edit to, say, `data/terms.ts`, merged to `main`, passes `tsc`/tests/build and LOOKS shipped — but `content_entries` doesn't update itself, so the live page keeps serving the old row until someone remembers to reseed. The script builds the authoritative 15,364 local rows, fetches only remote ids/kinds/content hashes, and upserts the stale or missing delta; a normal code-only push writes zero payloads. Seed jobs use a dedicated global concurrency group with `cancel-in-progress: false`, so they cannot overlap or be interrupted between batches; a newer successful seed queues and runs afterward. Obsolete stateless check jobs remain cancelable separately. **Needs two GitHub Actions repo secrets set by hand** (`NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, same values as `.env.local`) — not something an agent has permission to set. Until they're added the job fails harmlessly (doesn't block the build).
...
1. **Get the two GitHub Actions secrets set** (`NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`) so the `seed-content` CI job can run after pushes to `main`.
