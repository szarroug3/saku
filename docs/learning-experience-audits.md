...
This doc is meant to be self-sufficient — everything above (methodology, ground rules, the Workflow/agent-delegation execution shape, default scope) is the standing answer to "how do I run this." A kickoff prompt should NOT need to re-explain any of it. It only needs to supply what a static doc can't know in advance:

1. **A pointer to this doc** — "read `docs/learning-experience-audits.md` in full, then start."
2. **Anything time-sensitive or situational that's true right now but won't be true next time** — a background process already running (like a reseed), current git/branch state, a known WIP that changes today's constraints, anything that would otherwise need re-discovering from scratch. If nothing like that applies, the prompt needs nothing beyond point 1.

**Default scope, when a kickoff prompt doesn't specify which audits or what depth**: run all ten, at smoke-check depth (per each audit's own definition of that — e.g. Audit 1's "first N lessons," Audit 9's basics-only pass) for a first run, going deeper on whichever audit(s) turn up real findings rather than exhaustively deep-diving everything up front. If a kickoff prompt names specific audits or a specific depth, that overrides this default — but absent instruction, this is the standing behavior, not something to ask about before starting.

**Everything else — Workflow orchestration, agent delegation and context protection, the ground rules, the verify-stage techniques per audit — is fixed, standing behavior from the sections above, not something a kickoff prompt needs to restate.** If a kickoff prompt DOES restate something already covered here, this doc is the tie-breaker where they'd ever conflict, since it's the maintained, reusable source — a one-off prompt's wording shouldn't override a documented, load-bearing decision here.
