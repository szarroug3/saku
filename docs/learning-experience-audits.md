...
Kill anything that doesn't survive verification (majority vote across a small number of independent verifiers is a reasonable default) — except in Audit 10's 3-way-disagreement case, where "unresolved" is a valid, reportable outcome in its own right, not something to discard. Only survivors (confirmed bugs, confirmed clean, and explicit unresolved) reach synthesis.
