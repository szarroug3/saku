 ...
 *   3. each counter, in page order (人, 本, 匹, 枚, then the tail, then day-of-
 *      month and month-of-year last): its rule unit, followed by any memorised
 *      form it still keeps (only 歳's はたち, which no category can build — see
 *      COUNTER_GLYPH_BY_ID). The regular AND irregular counts are generated by
 ...
 *      "…, sai, day, month" — that trailing order is what puts them last here,
 *      with no bespoke step of their own).
