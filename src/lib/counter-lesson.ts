 ...
 *   4. day-of-month (〜日) and month-of-year (〜月), last: two more closed,
 *      memorised sets (see src/data/counters.ts, DAYS and MONTHS) with no
 *      generative category and so no unit — they are plain forms, scheduled
 *      after everything built on the counters/number system they use kanji from
 *      (their kanji prereqs — 日, 月 — resolve independently via each form's
 *      glyph; see directPrereqs in build-item.ts).
 * A pure function of the curriculum's order and the category list, built once.
 */
/** The counter glyphs scheduled as their own tail groups, step 4 above — not
 * tied to any generative unit, so not covered by COUNTER_GLYPH_BY_ID's splice.
 * In schedule order (day before month). Declared ahead of SCHEDULE itself
 * (below) because that export calls buildSchedule() eagerly at module init —
 * a const referenced there must already be initialized, not just hoisted. */
const TAIL_FORM_GLYPHS: readonly string[] = ["日", "月"];
...
function buildSchedule(): ScheduleStep[] {
  ...
  for (const glyph of TAIL_FORM_GLYPHS) {
    for (const form of CURRICULUM_COUNTERS) {
      if (form.counter === glyph) steps.push({ form });
    }
  }
  return steps;
}
