...
// eslint-rules/no-em-dash-in-user-facing-copy.mjs onto src/data and
// src/components (see that config for the documented per-file exceptions:
...
// project ESLint config, scoped to the SAME two trees, so `pnpm test` catches
...
describe("no em dash in user-facing copy (SAK-235)", () => {
  test("src/data and src/components stay free of em dashes outside the documented internal-note exceptions", async () => {
    const eslint = new ESLint({ cwd: REPO_ROOT });
    const results = await eslint.lintFiles(["src/data/**/*.{ts,tsx}", "src/components/**/*.{ts,tsx}"]);
