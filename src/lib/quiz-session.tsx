...
import { postClaim, postUnseen } from "@/lib/progress-fetch";
import { useFinishedRoundOutbox } from "@/lib/use-finished-round-outbox";
...
interface QuizSessionContextValue {
  ...
  endSession(): void;
  /**
   * Session complete → the learner's explicit choice: write history and
   * clear either way. `markKnown` (default false) — see the implementation
   * for the full SAK-52 rule.
   */
...
export function QuizSessionProvider({
  ...
  // The finished-round outbox — its own persistence channel, in its own hook
  // (use-finished-round-outbox.ts). `commitRecord` makes a finished round
  // durable; `saveError`/`retrySave` drive the banner. DELIBERATELY OUTSIDE the
  // snapshot: saveError is state about this device's conversation with the
  // server, not about the quiz, so it stays out of `canonical()` and out of the
  // save effect's deps — a field that changed on every failed retry and got
  // published to localStorage would be a new pump for exactly the cross-tab
  // write storm task 14 closed.
  const { saveError, retrySave, commitRecord } = useFinishedRoundOutbox();
  ...
  // ---------- history ----------
  //
  // The finished-round outbox — flushPending / retrySave / commitRecord and the
  // on-mount + back-online retry — lives in use-finished-round-outbox.ts. It is
  // wired at the top of the provider; `commitRecord` is called from the commit
  // paths below.
