...
// =========================
// The verb pool is derived from the corpus, and it is derived once, at build
// time: vehicles-build.ts holds the derivation, scripts/build-vehicles.mjs runs
// it into src/data/generated/vehicles.json, and this module reads that file.
// Everything below the pools is still live, because all of it depends on the
// recipe or the learner: which vehicles a recipe can take, which one a showing
// picks, what the dictionary says about a word. See vehicles-build.ts for what
// the derivation costs and why it moved (SAK-399).
...
/**
 * What src/data/generated/vehicles.json carries: the pools this module used to
 * derive from the corpus as it loaded, written once by
 * scripts/build-vehicles.mjs. `vehiclesVersion` is a content hash of the rest,
 * as on the other generated indexes.
 */
export interface BakedVehicles {
  readonly vehiclesVersion: string;
  readonly verbVehicles: readonly Vehicle[];
  readonly defaultVerb: Vehicle;
  readonly restrictedVerb: Readonly<Record<Transitivity, Vehicle>>;
}

const BAKED = vehiclesJson as unknown as BakedVehicles;
 ...
 */
export const VERB_VEHICLES: readonly Vehicle[] = BAKED.verbVehicles;
...
const DEFAULT_VERB: Vehicle = BAKED.defaultVerb;
 ...
 * 行く itself.
 */
const RESTRICTED_VERB: Readonly<Record<Transitivity, Vehicle>> = BAKED.restrictedVerb;
