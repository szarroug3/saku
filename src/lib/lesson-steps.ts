...
export function lessonSteps(
  ...
  let markedTsu = false;
  // Day-of-month and month-of-year no longer own a form-gated rule card here
  // (SAK-163 round 4): they are generative categories now, exactly like 〜本/
  // 〜人/the tens, so their rule card is handled by the generic marker branch
  // at the top of this function (constructionIntroForMarker), not by a
  // per-form check in this per-item loop.
