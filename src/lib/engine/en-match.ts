 ...
 * built (Datamuse rel_syn/ml, filtered) and self-reviewed. Read from disk
 * the first time an answer is matched, not carried by every bundle that
 * imports the engine (3.2 MB, SAK-399). */
function synonymPool(): Readonly<Record<string, readonly string[]>> {
  return readDataJson<Readonly<Record<string, readonly string[]>>>("en-synonyms.json");
}
