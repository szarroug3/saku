...
test("pronunciationUnitsOf — 人 splits by PRONUNCIATION: ひと / じん / にん, each its own unit", () => {
  const units = pronunciationUnitsOf(buildGlyphItem("人")!);
...
test("unitCost — counts meanings, not the reading; each の unit ≈ its senses", () => {
  const hito = pronunciationUnitsOf(buildGlyphItem("人")!).find((u) => u.reading === "ひと")!;
...
test("unitFrequency + byFrequencyDesc — 人's units order ひと > にん > じん", () => {
  ...
  const units = [...pronunciationUnitsOf(buildGlyphItem("人")!)].sort(byFrequencyDesc);
  assert.deepEqual(
...
test("pronunciationUnitsOf — every fact lands in exactly one unit (nothing dropped)", () => {
...
test("libraryOrder — a word appears ONCE, at its first (most-frequent) unit", () => {
  // 人 has three units (ひと/にん/じん) in the teaching order; the Library lists 人
  // once, positioned by ひと, and does not repeat it for にん/じん.
  const order = orderedUnits(["人", "三"]);
  const items = libraryOrder(order);
  const glyphs = items.map((i) => i.glyph);
  assert.deepEqual(glyphs, ["人", "三"], "one entry per item, no duplicates");
  assert.equal(new Set(glyphs).size, glyphs.length, "no item repeats");
});
