...
/** A quiz-question component (Stage 4 narrows this from `unknown`). */
export type FactRenderer = unknown;

/** ONE lesson viewport renders any item by looking its kind up here. */
export const itemRenderers: Registry<ContentKind, ItemRenderer> = createRegistry();
/** ONE quiz shell renders any fact by looking its kind up here. */
export const factRenderers: Registry<FactKind, FactRenderer> = createRegistry();
