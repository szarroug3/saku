...
export interface GrammarLesson {
  ...
  facts: FactId[];
  /**
   * Where you are, in SITTINGS — "lesson 14 of 30".
   *
   ...
   * 56 patterns would put "3–7 of 56" on a card that is really the 14th thing the
   * learner sits down to. `from === to` always: the item IS the current sitting,
   * so the span is a single number and `total` is the sitting count. The pattern
   * total (56) is still exported as GRAMMAR_CURRICULUM_TOTAL for callers that
   * count patterns rather than sittings.
   */
