...
//
// What the entry model says about an entry, as data: the labels a screen
// reads off a fact (subject, track, quiz track), which entries are speakable,
// which shelf each one lands on and which duplicates are kept off it, and the
// hand-authored lookalike pairs.
//
// The facts table and the "Built from" decomposition used to be tested here
// too. They served the old app's entry page, which is gone; their code left
// entries.ts with it (SAK-398).
