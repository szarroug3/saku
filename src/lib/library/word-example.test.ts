...
describe("the generated artifact", () => {
  ...
  test("covers exactly the words the corpus can cover", () => {
    ...
    // Tatoeba sentences, so they cover vocabulary too.
    assert.equal(covered, 2967, "corpus coverage moved; remeasure before editing this");
    ...
    assert.equal(EXAMPLE_COUNT, 2964);
  });
  ...
  test("every row is a real sentence containing its word", () => {
    ...
    // くすぐる is in くすぐらないで, 演ずる is in 演じている. 557 of the 2,964
    // rows do not contain their dictionary form literally and every one of them
    // is a conjugation, not a mismatch. What survives inflection in all 2,964 is
    ...
    // shift did, from a rank-order change, not a bug.
    assert.equal(literal, 2419);
  });
