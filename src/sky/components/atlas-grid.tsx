...
function Tile({ item, selected, onPick, onPeek, asName = false, big = true }: { item: SkyItem; selected: boolean; onPick: OnPick; onPeek?: (id: string) => void; asName?: boolean; /** Whether a one-character glyph is drawn large: only in a cut where every glyph is one character. */ big?: boolean }) {
  ...
  const lone = big && !name && [...item.glyph].length <= 1;
  return (
    <button
      type="button"
    ...
    >
      <span className={`max-w-full text-center leading-tight ${name && item.glyph === item.english ? "line-clamp-2 font-sky-ui text-[12.5px] font-semibold" : `truncate font-sky-display leading-none ${lone ? "text-[24px]" : "text-[15px]"}`} ${STANDING[item.standing].text} ${japaneseFont(item.glyph)}`}>{item.glyph}</span>
      {item.english !== item.glyph && <span className="max-w-full truncate text-[10.5px] text-sky-muted">{item.english}</span>}
    </button>
  );
}
...
export function TileGrid({ items, selected, onPick, onPeek }: TileGridProps) {
  ...
  // One cut, one look (Sam, 2026-09-21, on a Words cut where を stood at 24px
  // beside 顔を… at 15px: "even within words, the font is different"). A glyph
  // is drawn large only where every glyph in the cut is one character, which
  // is Kana, Radicals and Kanji. The line under it is the same small muted
  // line on a wide tile and a square one; the wide tile only shows more of it.
  const big = items.every((it) => [...it.glyph].length <= 1);
  return (
    <div className={`grid gap-1.5 ${wide ? "grid-cols-[repeat(auto-fill,130px)]" : "grid-cols-[repeat(auto-fill,72px)]"}`}>
      {items.map((it) => <Tile key={it.id} item={it} selected={selected.has(it.id)} onPick={onPick} onPeek={onPeek} asName={wide} big={big} />)}
    </div>
  );
}
