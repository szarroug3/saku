...
**A second pool, and the corpus still wins.** `scripts/ingest/word_example_candidates.py` reads the same three Tatoeba exports `grammar.py` reads, pinned by hash in `sources.json` and checked before a byte is used, and writes `word-example-candidates.json`: up to eight candidate sentences for each word the corpus never reached. `build-word-examples.ts` asks `chooseExample` for the corpus pool first and only falls through to the wider one when that pool is empty, so all 2,989 rows that existed before are byte for byte what they were, and the eight words whose every corpus candidate is banned keep their refusal rather than being quietly answered from somewhere else. **Coverage went from 2,989 words to 5,805** (23.8% to 46.2%), and in teaching order: 791 of the first 1,000 becomes 937 (79.1% to 93.7%), 1,763 of the first 3,000 becomes 2,449 (58.8% to 81.6%), and 430 of the first 500 becomes 484.
...
**Reading 150 of them, fifty from each band, is what settled the filters.** The first read found ten rows that were wrong sense, odd or too hard; after two rounds of tightening and seven named bans a fresh read of 150 finds four, and the table with a verdict on every row is in the card's closing comment. Two rounds of tightening came out of it: the meaning check now asks only about the meanings the page shows (JMdict's full sense list let 一人歩き, taught as "taking on a life of its own", keep a sentence translated "I like a solitary walk"), and the stopword list is stemmed, because without that "this" stemmed to "thi", matched nothing in any list, and let two rows through on the word "this". The seven a filter could not reach are in `WRONG_SENSE_EXAMPLES` with a line each: six are the 顔 case again, a sentence containing the word that teaches nothing about it, and each has a better sibling the chooser lands on once it is out of the way; 要する has only two candidates in the whole export, a frozen adverb and a proverb, so banning both leaves it as it was, without one. いただきます is in `EXTRA_EXAMPLES` instead, because the sentence that shows the phrase as the phrase names it in quotes and then explains it, which no nineteen-token filter would allow. The four still flagged in the read are a clumsy human translation, an arithmetic drill, a famous literary line whose なし the classical filter misses because UniDic files it as an adjective, and a correct sentence about regular expressions: real Japanese with real translations, each needing a human to say no.

**The size, and who pays it.** `word-examples.json` went from 576 KB to 1,145 KB, 1.99 times, just under the doubling that would have asked for a split. No client bundle carries any of it: the file is imported by `src/app/(sky)/teach.ts` and `src/lib/library/character-entry-content.ts`, both server-side, so it lands in one SSR chunk and `route_sizes.mjs` reports every route unchanged and inside its budget. The 1.1 MB candidate pool is a build input, read by the build script and its tests and by nothing at run time.
...
**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,116 unit tests, 4,115 pass and 1 skipped: the span pin now reads 5,805 rows with 4,726 dictionary spellings and 1,079 inflected ones, every one of them a form of its own word; a new pin says every row the wider pool supplied has a translation that shares a word with the meanings on the page, with twenty rows listed where the match needed English morphology the test deliberately does not do (ran/run, sold/sell, paid/pay); and `word-example.test.ts` checks the two pools separately, including that the wider one never answered for a word the corpus had covered. 66 e2e pass. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages.
