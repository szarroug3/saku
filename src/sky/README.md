...
## Example sentences for the words that had none (2026-09-17, SAK-461)

Sam, reading SAK-422 on いただきます in the Atlas: "it looks like these words no longer have a sentence example. is that intended?" It was never there. `word-examples.json` was picked out of `grammar-corpus.json`, and that file is not "every Tatoeba sentence": `scripts/ingest/grammar.py` keeps only sentences that also match one of `recipes.ts`'s grammar patterns and then caps each pattern at 200. So 9,566 of the 12,555 words had no "In a sentence" section at all, and a set phrase said before every meal was one of them.

**A second pool, and the corpus still wins.** `scripts/ingest/word_example_candidates.py` reads the same three Tatoeba exports `grammar.py` reads, pinned by hash in `sources.json` and checked before a byte is used, and writes `word-example-candidates.json`: up to eight candidate sentences for each word the corpus never reached. `build-word-examples.ts` asks `chooseExample` for the corpus pool first and only falls through to the wider one when that pool is empty, so all 2,989 rows that existed before are byte for byte what they were, and the eight words whose every corpus candidate is banned keep their refusal rather than being quietly answered from somewhere else. **Coverage went from 2,989 words to 5,803** (23.8% to 46.2%), and in teaching order: 791 of the first 1,000 becomes 936 (79.1% to 93.6%), 1,763 of the first 3,000 becomes 2,448 (58.8% to 81.6%), and 430 of the first 500 becomes 483.

**A word is matched on its spelling AND its reading, at a token boundary.** The old matcher had only `Example.v`, a list of written forms with the readings thrown away, and its header says plainly that it refused reading matches rather than guess: 後 is あと in this vocabulary and a 後 read ご is a different word. This pass has the sentences and fugashi in front of it, so it keeps only what agrees on both. A set phrase written in kana is the one case no token match can reach, because UniDic files いただきます as いただき + ます off the verb いただく and おはよう as an 感動詞; those match as whole strings that start at a token start and end at a token end, which is also how `sentence_readings.py` now finds the underline for them.

**Five filters, and what each one is for.** Every sentence needs a human English translation through the links export, a named owner with at least fifty Japanese sentences, at most fourteen real tokens, and plain Japanese script, all four imported from `grammar.py` rather than retyped. Then: no classical auxiliary, which is what drops the proverbs (流れる水は腐らず。, 光るもの必ずしも金ならず。); every other content word in the sentence taught by this app, by spelling or by reading, which is what drops 屏風, 虚言 and ティンホイル; no word harder than the word being taught, or than the 3,000th word in teaching order, whichever is later, which is what drops 保健体育の講義 from 体育's page while leaving 猟犬 its 嗅覚; the translation has to say what the page says the word means; and nothing vulgar or violent, with an exception for the word whose own meaning that is, 死ぬ and 戦争 and 殺す, but no exception for swearing, so くそ has no example rather than "Eat shit!".

**Reading 150 of them, fifty from each band, is what settled the filters.** The first read found ten rows that were wrong sense, odd or too hard; after two rounds of tightening and ten named bans a fresh read of 150 finds one, and the table with a verdict on every row is in the card's closing comment. Two rounds of tightening came out of it: the meaning check now asks only about the meanings the page shows (JMdict's full sense list let 一人歩き, taught as "taking on a life of its own", keep a sentence translated "I like a solitary walk"), and the stopword list is stemmed, because without that "this" stemmed to "thi", matched nothing in any list, and let two rows through on the word "this". The ten a filter could not reach are in `WRONG_SENSE_EXAMPLES` with a line each. Six are the 顔 case again, a sentence containing the word that teaches nothing about it, and each has a better sibling the chooser lands on once it is out of the way. 要する has only two candidates in the whole export, a frozen adverb and a proverb, so banning both leaves it as it was, without one. And three came out of Sam's review of the read: おっと, whose one candidate is glossed "Sorry, it's not good.", and 悔い, whose two candidates are the same classical line (我が生涯に一片の悔いなし。, which the classical-auxiliary filter cannot see because UniDic files なし as an adjective), both now have no sentence rather than that one; 表す swapped a sentence about regular expressions for 地図の青い線は川を表す。("The blue lines on the map represent rivers"). いただきます is in `EXTRA_EXAMPLES` instead, because the sentence that shows the phrase as the phrase names it in quotes and then explains it, which no nineteen-token filter would allow. One row in the read is still flagged and stays: 二の二倍は四。for 倍, which reads as an arithmetic drill.

**The size, and who pays it.** `word-examples.json` went from 576 KB to 1,144 KB, 1.99 times, just under the doubling that would have asked for a split. No client bundle carries any of it: the file is imported by `src/app/(sky)/teach.ts` and `src/lib/library/character-entry-content.ts`, both server-side, so it lands in one SSR chunk and `route_sizes.mjs` reports every route unchanged and inside its budget. The 1.1 MB candidate pool is a build input, read by the build script and its tests and by nothing at run time.

**No audio was seeded.** The wider pool adds 2,762 speakable strings that neither the corpus set nor the old word-examples set already covers, 16,572 clips across the six voices. `scripts/seed-voice-audio.mjs --set=word-examples --dry-run` is the count; seeding runs against the local Docker engine, after review, and not from here.

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,116 unit tests, 4,115 pass and 1 skipped: the span pin now reads 5,803 rows with 4,725 dictionary spellings and 1,078 inflected ones, every one of them a form of its own word; a new pin says every row the wider pool supplied has a translation that shares a word with the meanings on the page, with twenty rows listed where the match needed English morphology the test deliberately does not do (ran/run, sold/sell, paid/pay); and `word-example.test.ts` checks the two pools separately, including that the wider one never answered for a word the corpus had covered. 66 e2e pass. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages.

## The kana intro calls kana the Japanese alphabet and says "represent", not "spell" (2026-09-17, SAK-460)

Sam's reviewer on the Observatory's kana intro: call it the alphabet, explain that a consonant and a vowel share one character, and do not say "spell", which suggests several characters making one thing. Sam approved this text: "Kana are the Japanese alphabet: characters that stand for sounds, not meanings. Unlike English letters, most kana are a consonant and a vowel together in one character: か is "ka", not "k" and "a". There are two sets, hiragana and katakana, and they represent the same 46 sounds." "Phonetic alphabet" was left out because it also means the Alpha, Bravo, Charlie kind. Two other lines used "spell" for one character standing for one sound and now say "represents" (How Saku works, The three scripts) and "stand for" (the kanji intro). "Spell" stays where several characters make a word.

## Every word is now spoken from a text the engine was asked about first (2026-09-17, SAK-275)

The card asked for 589 clips to be listened to. Nobody listened to any of them, and nobody needed to: `POST /audio_query` reports the exact moras VOICEVOX will say before it says them, so all 11,543 readings the app can ask a voice for were checked by machine against the local Docker engine, and `scripts/build-speech-overrides.mjs --check` re-asks it any time.

**What the engine gets wrong from a bare reading.** It reads a は or へ inside a word as a particle (はちがつ comes out ワチガツ, じはだ comes out ジワダ), drops a consonant (しひ comes out シイ, へいれつ comes out エイレツ), cuts a word short (せんえんさつ comes out センエンサッ), and is inconsistent about long vowels: it smooths おう and leaves えい literal, so 学校 was already "gakkoo" while 先生 was "sense-i". The other direction is wrong too, where the う is a verb's own ending: 囲う is "kakou", not "kakoo".

**How "right" is decided, per word.** The expected sounds start as the reading in katakana, one mora at a time, because the consonants of a taught reading are not negotiable. Exactly two positions may differ: a long-vowel site (an お-vowel mora before ウ, an え-vowel mora before イ) and a real particle は or へ. Which way each one goes is decided by the word written normally, since a kanji spelling anchors the engine's dictionary lookup, and only when that spelling agrees with the taught reading beat for beat everywhere else. That guard is what keeps the known liars out: 栄え's spelling reads さかえ, 幅広's reads はばひろ without the taught rendaku, 時は金なり's reads きんなり. Those, and 54 more, were decided by hand in `HAND_DECIDED`, each with its reason, and the script refuses a hand answer that changes a mora it is not allowed to. A word with no kanji spelling (こんにちは, うとうと, エイズ) has no second witness and needs none: its normal spelling is the reading, so the engine's own answer stands. That is also why this could not break こんにちは the way a blanket katakana swap once did.

**Then a ladder, and nothing is assumed.** The reading as it is, the reading in katakana, hiragana with ー at the smoothed sites, hiragana with え/お, the same two in katakana, and last the word written normally. The first one whose engine moras match wins, and it also has to keep the word in as few accent phrases as the reading or its own spelling does, because ホントウ comes back as ホン + トオ, a pause inside 本当. Every rung earns its place: katakana fixes 8月 and 私費 and 千円札 but not 性癖; せえへき fixes 性癖 and へーえき fixes 兵役, both of which the previous pass had written off as unfixable; only the kanji fixes 使う and 襲う.

**The result: 853 readings now send something other than themselves**, 795 for a long vowel, 45 for a wrong consonant, 13 for both. 10,471 were already right and are untouched. Nothing is left broken. Ten readings get no override because two words share them and want opposite sounds: かこう is both 囲う "kakou" and 加工 "kakoo", and one clip cannot be both, so seven of the card's eight う-ending verbs stay as they are rather than breaking their homophones. They are named in the closing comment on the card.

**The hand list is gone, the table is generated.** `CONFIRMED_BAD_READINGS` is now the keys of `src/data/generated/speech-overrides.json`. All 95 readings SAK-215/218/219/243 confirmed one at a time are still overridden, pinned by `src/lib/speech-overrides.test.ts`. 76 of them send the exact same text as before. Of the 19 that do not, 13 sound identical and only stopped using a katakana twin that split the word in two, and 6 sound different because they were still wrong: the five へ words whose katakana dropped the ヘ, and 競争入札, whose ending was still being cut off.

**No audio was seeded.** The Storage path is a hash of the text the caller asked for, not of the text sent to the engine, so every path stays where it is and the clip at it is stale. 764 readings change how they sound, which is 12,414 clips across the six voices: 4,584 general (`words` and `word-readings` share a path), 42 `yomi`, 7,824 `pitch` and 3,996 `lesson-pitch`, overlapping to 12,414 distinct paths. They have to be deleted before a re-seed, because seeding skips a path that already exists. Seeding runs against the local Docker engine, after review, and not from here.

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,125 unit tests, 4,124 pass and 1 skipped: eight new ones pin the table's size, its named rows, that every key is a reading some word is taught under, that no override sends the reading back unchanged, that a kana override keeps the reading's own number of beats (which the pitch clips count on), and the 95-reading regression list. 66 e2e pass. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages.

**Replacing the stale clips without deleting them (SAK-275).** `scripts/seed-voice-audio.mjs --refresh-overridden` makes again every clip whose text has an entry in `speech-overrides.json` and writes it over the old one at the same path (the upload was already an upsert), once per path per run. 14,022 clips over six voices. Nothing is deleted first, so a learner never meets a missing clip while the run is under way, and the run can be stopped and started again.

## The stored voice clips can be compared with what the app asks for (2026-09-17, SAK-463)

`scripts/audit-voice-clips.mjs` lists the voice bucket and the seeding sets and reports both differences: clips stored that no set lists any more, and clips a set lists that are not stored. Report only by default; the unwanted paths go to `.logs/voice-clips-unwanted.txt`. `--delete-unwanted --execute` removes them and is run by Sam, not by an agent. A clip the live route recorded for a string no set lists looks the same as a leftover, so the count is "at most this many". First run: 251,541 stored, 251,364 wanted, 177 unwanted, 0 missing.

## Eleven words that cannot share a reading's clip now have their own (2026-09-17, SAK-462)

A clip lives at a hash of the text the app asked for, and the app asks for a word's kana reading, so one reading is one clip shared by every word read that way. That is right for a homophone pair and wrong for ten readings where the two words are not homophones at all: かこう is 囲う, said "kakou" because the う is the verb's own ending, and 加工, said "kakoo" with a long o. SAK-275 could fix neither without breaking the other, so it left all ten alone and named them. This fixes them.

**The word the shared clip says wrong sends its own written form instead.** 囲う sends 囲う, which the engine reads カコウ; 加工 goes on sending かこう, which it reads カコオ. The spoken text differs, so the hash differs, so the path differs, with no change at all to how a path is worked out. The answers are in `src/data/generated/speech-word-overrides.json`, keyed by the written form and the reading together, and `src/lib/speech-text.ts`'s `speechTextFor(word, reading)` is the one place that reads it. A caller with no single word in hand (a kanji's on'yomi row, a sentence, a bare kana) passes none and gets the reading back untouched.

**Before and after, per word.** Six readings where the verb was the one said wrong: 囲う カコオ → カコウ, 沿う and 添う ソオ → ソウ, 憩う イコオ → イコウ, 酔う ヨオ → ヨウ, 問う トオ → トウ, 乞う コオ → コウ. Their partners keep exactly what they had: 加工 下降 河口 カコオ, 想 総 ソオ, 以降 意向 イコオ, 用 ヨオ, 党 唐 塔 トオ, and the plain adverb こう コオ. Four readings the other way round, where the verb was already right and the other word was not: 火曜 カヨウ → カヨオ, 疲労 ヒロウ → ヒロオ, 野党 ヤトウ → ヤトオ, 仁王 ニオウ → ニオオ, while 通う カヨウ, 拾う ヒロウ, 雇う ヤトウ and 匂う ニオウ keep theirs. 陽, 頭 and 侯 answer a different word entirely from their spelling (ヒ, アタマ, ホオ), so they never had a vote and keep the shared clip.

**Who has to know which word is asking.** `speechForFact` already did, for every fact that is a word. The hear button takes a `word` prop beside the reading it says, and the lesson card fills it in for the rows that are one word (the head, a form, one of a word's other readings) and leaves it off for a kanji's reading rows. A quiz choice carries the word it is a clip of, which matters on a pitch card whose two choices are two different words: 憩う against 以降 is a pair the pitch quiz really asks, and its two clips were the same wrong clip at two pitches. The four seed sets that speak a word (`words`, `word-readings`, `pitch`, `lesson-pitch`) walk VOCAB and already have the written form in hand.

**The generator does the deciding, and proves it.** `scripts/build-speech-overrides.mjs` already worked out what each word should sound like and which readings clash. On a clash it now splits the words by the sounds they want: the ones the reading sent bare already says keep the shared clip, and every other one is given its own written form, which has to say the expected moras and keep the word in one accent phrase or it is reported as a clash and left alone. Zero are left. `--check` re-asks the engine for both tables. The reading-level table is byte-identical to what SAK-275 shipped.

**Nothing else moved.** Every path the four sets ask for was dumped before and after: 251,364 distinct clips became 251,490, with 138 added and 12 gone, and not one of the other 251,352 changed. `src/lib/speech-text.test.ts` pins the ten readings with the moras each side wants, that each pair resolves to two different paths for both the general and the pitch cache, and that every other word in the whole vocabulary still speaks its reading unchanged.

**Seeding, for Sam, not from here.** 138 clips are missing: 66 general (`words` and `word-readings` share a path) and 72 pitch (`lesson-pitch`'s 36 are `pitch`'s own). `scripts/seed-voice-audio.mjs --set=all` picks them all up as ordinary cache misses; `--refresh-overridden` is not the flag for this and would not find them, because these paths hold nothing to refresh. Twelve stored clips become leftovers, いこう:2 and やとう:0 across the six voices, which were 憩う's and 野党's alone and which `scripts/audit-voice-clips.mjs` will list as unwanted.

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,133 unit tests, 4,132 pass and 1 skipped, with eight new ones on the table. 66 e2e pass. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages. `build-speech-overrides.mjs --check` passes on 853 overrides and 11 words with a clip of their own.

## A reference page nobody has read opens the lesson (2026-09-17, SAK-467)

Sam: "when an unseen before reference page is in a lesson, when the lesson opens, it should show the first reference page." Three rules, and they pull apart cleanly: what is listed, what opens, and what counts as read.

**Every reference is listed every time.** References is the list of what tonight rests on, and what tonight rests on is a property of the order, not of the reader. The app's teaching walk drops a card whose track the learner has already touched (`startedTracks`), which is right for a drill and wrong here: the three kana terms behind the vowels were listed for the first two rows of hiragana, then gone from the s-row onward and never back. `lessonSteps` takes a fourth argument now, `everyCard`, and `walkFor` in `src/app/(sky)/lesson.ts` passes it: no track counts as started and no yōon card counts as met, so the walk plans every card it owes. History is still handed in, so where each page goes in the order is worked out exactly as before, and a spine card that rides a later item for a learner with progress still rides it.

**A page nobody has read opens the lesson.** `unseenPages` in `src/sky/lib/lesson.ts` is the pages of the list the learner has never been shown, in the order the panel lists them. `SkyLesson` takes them as `openPages` and walks them ahead of the order: the first one is what is showing when the lesson opens, Next moves to the next one and then to step one. A star already in the sky is never one of them, since it is there to look at rather than to read. A lesson picked up through Continue is handed none and opens where it was left (SAK-444); so are the pretend learner and the showcase, which keep nothing and are a look at the whole lesson.

**The counter does not move while a page is showing.** It reads "Step 1 of 5" from the moment the lesson opens, through every page of the lead, and still reads "Step 1 of 5" when the first star opens. That is what it already read for any other reference opened mid-lesson, and it is the SAK-416/446 rule intact: the pages are never steps, so N never grows and n never drops to zero. Which page of the lead the lesson is on is read off what is showing rather than kept beside it, so opening one from the panel puts the lesson back on the lead there and Next carries on from it.

**A page counts as read once it has been in a lesson.** Every reference page of tonight's lesson is marked the moment the lesson opens, clicked or not: the panel is beside the sky the whole time, and a lesson that opened on it has done its part. The mark is kept in the settings blob, this browser's localStorage signed out and the account's `settings` column signed in, which is the store the old app's "intro already shown" mark used and the same question. It is deliberately not history: history is what a learner answered or claimed, it feeds the schedule and the standings, and a page read is neither. `src/app/(sky)/pages-seen.ts` reads it once per page load, for the reason the lesson's place is read once: the lesson is the writer as well as the reader.

**What it looks like.** The vowels lesson on first open shows the Kana term page with its row outlined in References, "Step 1 of 5" over it and あ standing as the first step. Three presses of Next walk Kana, Hiragana and Romaji, then open あ, with the counter on "Step 1 of 5" throughout. The k-row lesson after it lists the same three pages with none picked out, and opens on か at "Step 1 of 10".

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,142 unit tests, 4,141 pass and 1 skipped, with nine new ones: five on which pages a lesson opens on, three on a page still being listed for a learner who has walked another row or claimed one, and one on the settings blob carrying the mark both ways. 67 e2e pass, one of them new. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages.

## A particle is a moon, a grammar pattern a comet, and only a sentence type is a planet (2026-09-17, SAK-465)

Every grammar pattern and every sentence type used to be a ringed planet, so five particles picked for one night were five planets in one cluster: the biggest body in the sky drawn on its most ordinary thing. Sam: "planets should be rarer". How many of a thing there are now decides the body.

**The three bodies.** `Body` gains "moon" and "comet", and `bodyOf` reads a kind plus one flag: a particle is a moon, any other grammar pattern is a comet, a sentence type is still a planet. Counters, verb pairs and everything else are as they were. `BODY_ORDER` is the one list of the six, rarest first, and the home walks it to decide which body to open the sky on: a planet, then a binary, then an asteroid, then a comet, then a moon, and never a star, because nearly all of the sky is stars.

**How a particle reaches the sky.** `src/sky` cannot import from `@/data`, so the item carries the answer: `SkyItem.particle`, set by the Observatory's picker, which is the one place a grammar item is built. Which recipes are particles is not decided again here: `PARTICLE_RECIPE_IDS` in `src/lib/library/grammar-shelf.ts` is the Library's own particle section (は が に で を へ まで までに だけ しか〜ない か), now exported, so there is one answer to "is this a particle" rather than two lists that can drift. A flag and not a kind of its own: a particle is picked, gated, quizzed and taught exactly as any other pattern is, and only the drawing differs.

**What they look like.** Both are flat, two toned and free of gradients, like the planet: one tone is the standing's own color, so the body still says how it is going, and the other is either the night behind it or starlight. The moon is a disc three times a word star wide with a crescent of night lying along one limb. The comet is a bright head with a tail tapering to a point away from the middle of its own constellation; a comet at that middle takes a direction seeded by its id, so it is the same on every sky and on the server and in the browser alike. Reaches follow: a moon's is its disc, a comet's is the tip of its tail, so the packing keeps its spacing and nothing is unclickable.

**The sizes were looked at, not guessed.** Six moons and five comets were drawn side by side at 0.7, 1 and 1.8, the three scales the sky uses, and the pictures were opened. The first moon (radius 8, a shadow reaching almost halfway across) read as a blob at 0.7 and was dropped for radius 10 with a shadow that leaves a little over half the disc lit. The first comet was too faint and too short, and the head went from 4.6 to 5.5 and the tail from 15 to 18. At 0.7 a moon is 14 screen pixels across, and both read as what they are.

**The catalogue was rebuilt.** `src/data/generated/sky-catalogue.json` holds every star without its standing, so a new field on an item is a new field there too; `npm run build:catalogues` was rerun and the payload's `extras` is back to empty.

**What did not follow.** The legend's key names the six standings and tonight, not the bodies, so nothing there changed, and "How Saku works" never names a body either. Adding a key of the bodies would be a page of its own and is not this card.

**One thing this did not fix.** A lesson sky whose picks are few draws some of its constellations below the pane, which is short and wide, and they are clipped. It is not new: the boxes a constellation is packed into are SMALLER now (60 down to 56 for a grammar pick, since a comet and a moon reach less far than a planet did), so if anything there is more room than before.

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,135 unit tests, 4,134 pass and 1 is skipped. Two are new, every body listed once in `BODY_ORDER` and given a reach, and a comet's tail pointing away from the middle and the same way every time; the one that pinned a grammar pattern to a planet now walks every kind, the particle flag included. 68 e2e pass, two of them new: the Observatory's "Your sky tonight" with the five particles picked draws five moons and no planet, and a lesson sky with 〜てから draws a comet. `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages.

## A tile that cannot be taken yet is not on the Observatory at all (2026-09-17, SAK-464)

The "Sentence rules" row ended on a dim "Simple" tile reading "Opens once you know は or が". Sam: "let's not show things that aren't unlocked ... don't show 'Opens once you know は or が' or anything like that on the cards. just let it open when it opens."

**What a tile is waiting on is a list of ids, not a sentence.** `ItemGate`, the one-line "what opens this" the card carried, is gone, and with it the `gates` map on a section and the dim look on `ItemCard`. In its place the section says what each of its rows is still waiting on, by id (`needs`, from `waitingOn` in `src/app/(sky)/observatory.ts`), and the page draws a row only when every one of them is learned or picked tonight. Picking は and が brings "Simple" into the row, and taking either back out takes it away again, with the type's own pick removed too: `settled` does for a lost requirement what `withoutPick` has always done for a lost part.

**Why it is the section's and not the item's.** The obvious move is to make those patterns the type's `components` and let the cart's own `pickState` hide it. It cannot be: the home ships one catalogue of the whole sky to every learner and builds its graph once, so a star's parts have to be the same for everybody. Hanging one learner's place in the order on the star made `skyPayloadFor` disagree with `splitSky` for the sample learner, which `sky-payload.test.ts` caught.

**The rule it reads is still the app's own.** The lesson planner's `sentenceTierBlock` wants ANY ONE of a type's patterns, so a type with one of them already met waits on nothing and is an ordinary tile. A type with none of them waits on all of them, which is what Sam asked for ("don't show simple until either both are learned or both are selected") and is only ever stricter than the planner, so the Observatory can still never offer a type a lesson would then refuse to build. The other half of that rule, too few sentences of a shape to drill, is nothing a pick can change, so a type shut on it is left out of the section and the row ends on the patterns before it.
...
The Sentences row read 〜な, は, が, を, に, で, だけ, and then Simple: seven lessons before the one they are all for. Sam: "why isn't simple sentences not after topic/subject? why does it come after all these other particles", and, on 〜な, "if that's grammar but is required, it's the first thing taught in grammar iirc. you can lock the sentence track behind learning it in the grammar track."

**What a type requires, and what its examples merely use.** Those are two different lists and the order used to run them together. A Simple sentence IS a topic or a subject and a predicate, so は and が come before it. を, に, で and だけ are what its eight curated sentences happen to turn on, so they come after it. `sentenceRuleOrder()` places the prereqs, then the type, then the patterns its own readable sentences use, most used first, and the first three types now read:

    は  が  [Simple sentences]  を  に  で  だけ
    〜て  〜ている  〜てから  〜てしまう  〜てみる  〜ておく  [Te-form links and helpers]
    〜てください  〜ない  〜ないでください  〜ます  〜ましょう  [Requests and proposals]

**Every step says which type it is for.** A type no longer stands at the end of its own run, so "which type does this pattern belong to" cannot be read off the position any more. Each pattern step carries the tier it was placed for, and a leftover carries none. The Atlas's Sentences shelf cuts by that, so を is still filed under "Simple sentences" though it comes after it, and the Observatory's row runs to where the next type's run starts rather than to the type itself, which is how を, に, で and だけ stay on the row with the type they are for.

**A type's page shows the examples a learner can read.** Every curated walk example now carries `p`, the patterns it turns on, named after the same field on an assembly item and read the same way, with one difference: the corpus lists the patterns that CLASSIFY a sentence, so a Simple item is `["wo"]` and its は goes unsaid, while these list every pattern the line uses, because the question here is whether the learner can read it. `readableTierExamples` keeps the ones whose patterns are all learned, claimed, or picked for tonight, which is SAK-464's sense of "in the sky or among tonight's picks", and the page says nothing about the rest. The Atlas entry passes what the learner has; the lesson passes that plus tonight's picks.

**Only Simple falls back, and this is why.** Asked whether each type has an example readable from its `grammarPrereqs` alone, the answer is none of the ten: every curated example uses は, and no tier but Simple lists は as a prereq. Asked the question a learner actually faces, "can I read these at the point in the order where I meet this type", nine of the ten have all three examples readable. Simple is the exception: all three of its examples turn on を, which now comes after it. So a page with nothing readable shows the examples that are missing the fewest patterns instead of an empty step, which for Simple is all three, one を away.

**〜な moved to a Grammar row.** The Observatory has two grammar rows now and they divide the track exactly once. Sentences is one sentence type with the patterns it needs around it. Grammar is everything no sentence type ever asks for, in the track's own teaching order, which is the tail of the same list. 〜な heads that tail because `CURRICULUM_PATTERNS` starts with `prenominal-form`, so the row leads with the first lesson the grammar track teaches, read off the track rather than named here.

**One rule for the row and for the rows in it.** The Sentences row waits on 〜な under the row's own id in the same `needs` map its tiles use, and every tile in the row waits on it too. So the whole row is off the page until 〜な is learned, claimed, or picked tonight, the page's own `waited` answers both questions with one function, and taking 〜な back out takes the row's picks with it, the way losing は already took Simple.

**What was left alone.** The intro page's headline sentence is the guide's own, the one its paragraphs are about rather than one of a list, so it is not filtered. The section copy for Sentences kept its words and Grammar got its own, which say what a pattern is rather than what a sentence is.

**The gates.** `npx tsc --noEmit` and `npx eslint src e2e scripts` clean. 4,177 unit tests, 4,176 pass and 1 skipped, with eight new ones: four on the Grammar row and the row that waits on it, four on the examples a page shows. 72 e2e pass, one of them new: signed out with kana done there is no Sentences row at all, picking 〜な in Grammar brings it in with は first, and は and が make Simple the third tile, before "marks the direct object". `scripts/unreachable.mjs --list` at zero, `scripts/unused-exports.mjs` at zero on both lists, and `scripts/button-centering.mjs` at zero over 1px across 7 pages. `npm run build:catalogues` was rerun, since the Atlas's Sentences shelf is cut by the order.

## は and が get a page that says what they mean (2026-09-17, SAK-470)

The app taught は in five words: take a noun, add は. That is the build, and nobody is stuck on the build. Sam, on Tofugu's は/が article: "the pages on tofugu explain these topics really really well. can we distill some of that knowledge into our pages?"

**Its own page, not more prose under the table.** A pattern's pager was 〜は | Family. It is 〜は | What は does | Family now, so the build page stays the build page and a learner who wants to know what the particle is turns one page. `particleNotePage` in `src/app/(sky)/teach.ts` builds it from `src/data/grammar/particle-notes.ts` and `grammarPages` puts it between the two. A particle with no note written yet gets no page, which is how を, に and で will arrive.

**は and が end with the same section.** Neither can be explained without the other, so `WA_GA` is one run of prose both pages print, with the first paragraph taking the section's heading. It defines topic and subject in a line each, walks 私は学生です against 私が学生です, gives the question-word rule and why the answer keeps が, and closes on three mistakes. The Read more link is the note's own, so each page names the article once, under its own title.

**Which is why the Family page links nothing now.** The wa-ga cluster's link was the same Tofugu article, one page further along the same pager, and two links to one article on pages a learner turns between is one too many. The slot is null with a `noLinkReason`, which is the record `clusters.ts` keeps of every empty one. Its `feel` note used to say there was no rule for choosing between は and が; that stopped being true when the page before it started saying what each one does, so it points there instead.

**A paragraph can carry its sentence.** `TeachParagraph` gained `examples`: the Japanese as runs with the particle in the accent, the English under it, set in behind a rule down the left. It is a list because one idea is sometimes the difference between two sentences, 猫は好きです beside 猫が好きです, and a page that spent an empty paragraph on the second would read as a gap. Every occurrence of the particle is marked, not the first, so both は of 夏は暑いですが、冬は寒いです are picked out.

**How は is read is said here too, in the kana cards' own words.** は is "ha" everywhere else and "wa" as the topic particle, which a learner sees during hiragana (`PARTICLE_RULE` in `phase-intros.ts`, `NOTES` in `characters.ts`) and needs again the first time they read a particle page. The wording matches those cards rather than being written fresh, and a test pins that it does. へ and を get the same line when their pages come.

**Nothing is Tofugu's.** Their page was read for what a page has to cover and in what order, and every sentence here is Saku's own. The check: both texts to plain lowercase words, then the longest run they share. Four words, all of it ordinary English ("is doing the verb", "part of the sentence"). The only longer match anywhere is the article's title in the Read more line, which is the citation. The example sentences are Saku's own authored rows (私は学生です, 猫が好きです) or written fresh out of beginner words.
...
## The kanji card's Readings fold: words with their readings, on'yomi under its own heading (SAK-482)

From the bare-kanji audit of 2026-09-25. Two things in the Readings fold of a kanji card.

**The words have furigana.** The fold listed up to four words for each reading as plain text (期日 休日 近日 元日 beside じつ). Each word now comes across as `{ word, sound }`, where `sound` is a `SoundLine`, and the card draws it with `Sound`, the same ruby renderer the "In a sentence" block uses. `wordSound` in `src/app/(sky)/teach.ts` builds the runs from the word's vocab row through `piecesOf` (`src/lib/library/word-pieces.ts`), so each kanji carries its own reading in that word: 休日 is きゅう over 休 and じつ over 日, and the kana in a word such as お誕生日おめでとうございます print as they are. A word that does not split by kanji gets its reading over the whole word, and a word the vocabulary does not have prints plain. As shipped every one of the 9,208 example words splits by kanji, so neither of those two cases shows up yet. Each word sits in its own box so it never breaks across two lines, and the words' line height leaves room for the readings over them when the list wraps.

**On'yomi is under On'yomi.** The fold decided on or kun by looking for katakana in the reading, and the readings arrive in hiragana, so every reading of every kanji was listed under "Kun'yomi · the native reading", にち and じつ on 日 among them. `readingKind` now reads the kanji row's own `on` and `kun` lists from KANJIDIC2. The reading row's `type` field says the same thing for all 3,496 rows (checked while making this change), and it answers for a reading on neither list, which as shipped is none. A reading on both lists (20 rows, such as き on 気 and し on 死) is `kind: "both"` and the card shows it under both headings, the way `src/lib/kanji-onyomi.ts` already treats those rows.

**Tests.** `teach.test.ts` checks that 日's にち and じつ are on'yomi and ひ is kun'yomi, that 休日 comes across as 休 with きゅう and 日 with じつ, that every word on 日 spells itself back from its runs with a kana reading over each kanji and nothing over the kana, and that no word on ten common kanji is left without a reading. The e2e test opens 日's Readings fold in a lesson and checks that にち and じつ come before the Kun'yomi heading, ひ after it, and that 休日's readings are きゅう and じつ.
## Furigana where the readings already existed (SAK-481)

From the bare-kanji audit of 2026-09-25. Three places printed kanji with no reading over them while the readings sat in the data.

**The word card's "In a sentence" fold.** Every word's sentence was stored with one reading per kanji (`kr` in `src/data/word-examples.ts`, SAK-95), and `teach.ts` copied only the sentence, its English and its span. It now passes the readings as `example.sound`, built by `slotRuby` in `src/data/sentence-readings.ts`, which is the SAK-480 run builder with the slots handed in rather than looked up. `ExampleSentence` in `lesson-card.tsx` draws the sentence with `Sound` and cuts the runs at the span with `cutLine` (`src/sky/lib/sound-line.ts`), so the underline goes around 仕事 with し and ごと over it. Of the 5,622 word sentences with kanji, 5,570 now carry readings. The other 52 have no kanji the readings pass was sure of and print as before, and across all of them 1,192 of 22,532 kanji (about 5%) stay bare for the same reason.

**The Particle page's table.** Its 13 sentences are the ones the "In a sentence" block shows, so `withParticleMarked` reads them with `sentenceRuby` and picks the particle out of the runs with the same `cutLine`. A particle is kana, so the cut never lands inside a reading.

**The grammar drill's reveal.** A card drilled on a word the learner knows draws it in kanji, and "How it is built" and "Why the others were there" printed 選 bare. The route now draws the same showing a second time in kana (the vehicle with `known: false`, which changes only how it is drawn) and lines the two up word by word: the kana between the kanji has to match in both spellings, and what is left over the kanji is its reading. That is what puts き over 来 in 来て and く over 来 in 来る. `QuizCard.built` is a list of `SoundLine`s now, and a choice carries `sound` for the why-list only; the choices themselves stay plain while the card is open. Checked over 8,910 cards drilled on every vehicle in the pool: every line and every listed choice came out with its reading. A line the two spellings do not agree on would print plain.

Two things the card did not say. The drill puts one reading over a run of kanji (元気 げんき), where the sentences put one over each kanji; the vehicles are single words, so the run is the word. And the reading over a known word shows only after the answer, as the card says; the choice buttons and the prompt are unchanged.

**Tests.** `teach.test.ts`: 仕事's sentence reaches the card with いま, し and ごと; the first 300 words' sentences spell themselves and put kana over kanji only; every Particle table sentence has a reading over every kanji and still picks out its particle. `quiz.test.ts`: えら over 選 in 選んで's built line and in every listed choice, 来 read く and き by its form, and a card drawn in kana has no readings. `sound-line.test.ts` covers the cut. The e2e test opens 仕事 in the Atlas and checks three `<ruby>` with their `<rt>` in the sentence, the reading centered over its kanji; the SAK-443 underline test reads the text without the readings.

## Furigana on word chips and conjugation tables (SAK-483)

From the bare-kanji audit of 2026-09-25 (findings B3 and B4). Two places on a card printed words with kanji and no reading.

**Word chips show their reading beside the word.** The chips under "Words written with it" and "Words you know that use it" on a kanji or radical card, and under "Part of" on a lesson card, printed 今日, 真面目 and 申し込み bare. `StarButton` in `src/sky/components/lesson-card.tsx` now fills its `note` slot with the word's reading when the caller gives no note of its own, so a chip reads 今日 きょう today, the same way "Written with" prints a kanji's reading beside it. The rule is `chipReading` in `src/sky/lib/japanese.ts`: a word written with kanji whose reading differs from the word. A kanji or radical chip keeps its glyph and English gloss and nothing else, and a word written in kana is its own reading. I chose the note over furigana because the chip's glyph is 18px, so a reading over it comes out near 10px, and a row of chips where some carry a reading over the word and some do not would be two heights. The chip only has the word's whole reading (`SkyItem.reading`) to work with, which is the other reason: the note needs nothing more, where furigana over 申し込み needs a reading per kanji the chip does not have.

**Each form in a verb or adjective card's tables has furigana.** The "Written" column of the forms tables (15 to 22 cells on a card) printed 食べます, 食べさせられる and 大きくなかった bare. `formsOfWord` in `src/lib/word-forms.ts` now builds every form from the word's kana too (`reading` on each row: たべさせられる beside 食べさせられる), and `formSound` in `src/app/(sky)/teach.ts` turns the pair into a `SoundLine` that `Table` draws with `Sound`, the same ruby renderer the Readings fold and the "In a sentence" block use. Conjugation only changes the kana after the last kanji, so the dictionary spelling up to its last kanji (食, or 申し込 in 申し込む) is fixed; what follows it in the form is the kana the form adds, the kana spelling ends with the same kana, and what comes before them is what the stem says in this form. When that matches the word's alignment from `piecesOf`, each kanji takes its own reading (た over 食; もう over 申 and こ over 込, with the し between them plain). Two more cases get a reading, both taken from the form itself rather than guessed: a stem of one kanji that says something else in this form (来 is こ in 来ない and き in 来ます), and a word that does not split by kanji, which carries the stem's reading over the whole stem (まね over 真似 in 真似ます), the way `wordSound` treats such a word. Anything else prints plain.

Measured over the vocabulary: 2,326 words written with kanji have forms, 36,307 cells in all, and 12 of them print plain, all on 射る and 鋳る. Both are read いる, and the conjugation engine refuses some forms of the kana spelling いる because the existence verb いる has none, so those cells have no kana form to take a reading from.

`Table` in `src/sky/components/teach-page.tsx` draws a cell that carries furigana at 17px in the display face, the size of the "In a sentence" block, and puts every cell in that row on one baseline. At the table's own 13.5px the reading over 食 came out near 7px. No other table in the Sky carries furigana yet, so this changes only the forms tables.

**Tests.** `teach.test.ts` checks that 食べさせられる is た over 食 then べさせられる plain, that every form of 食べる, 言う, 大きい and 汚れる has a kana reading over its kanji and nothing over its kana, the 申し込みます, 来ない, 来ます and 真似ます cells, that every word chip on 日 has a reading, and that the kanji chips on the 日 radical have none. `japanese.test.ts` checks `chipReading` on its own. The e2e tests open 食べる in the Atlas, open "Plain and polite" and find た in a `<ruby>` over 食べます and one reading in every cell, and open 日, where the 今日 chip reads きょう and a kanji chip has no kana on it.
## Furigana where the readings had to be written (SAK-484)

From the bare-kanji audit of 2026-09-25. Six places printed kanji with no reading over them, and no reading for them existed anywhere in the data. Each now draws its Japanese with the same `Sound` ruby renderer the "In a sentence" block uses.

**How prose gets its readings.** A page of teaching is English with Japanese in it. `kanjiRunsIn` and `rubyProse` in `src/sky/lib/sound-line.ts` split a line into its runs of Japanese (猫は好きです in "猫は好きです is about cats" is one run) and ask a reader for each run that has a kanji in it. `src/app/(sky)/prose-sound.ts` has the two readers. Grammar prose reads a run as one word from the vocabulary first (`wordSound`, moved here from `teach.ts`), and anything else from the readings pass (`sentence-readings.json`). A term's page reads only what its own card wrote. A run neither answers for prints as it did.

**1. A particle's page.** Each paragraph of `particle-notes.ts` now reaches the page with `runs`: 食べる, 飲む, 誰 and 今日 from the vocabulary, 猫は好きです, 田中さんは来ました, 学校に歩きます, 水しか飲みます, 部屋でいます and 安くておいしいです from the readings pass. The same reader covers a pattern's own pages and a sentence type's walk, which is how と思う on the volitional page and on "I think / seems like" got theirs.

**2. The Family table.** The Pattern and Built cells go through the grammar reader: 行くから, 行ってから, 本は, 本は車より, and the senses written in kanji (〜そう 様態, 〜から 理由).

**3. Term pages.** A card (`PhaseIntro`) and a term (`Term`) can now carry `readings`: each run of Japanese as it is written in the card's text, and its whole reading in kana. `rubyFromReading` matches the run's kana against the reading and gives the kanji what is left, so 見る みる puts み over 見, 生まれる うまれる puts う over 生, and 時々 ときどき puts the whole reading over 時々, which is the 々 page's point. A run written as null prints plain on purpose: a shape rather than a word (氵, 亻, 忄, 辶, 气, 氺, 々 on its own, and 泳 named as a shape beside "swim"), or a kanji the paragraph is about because it has more than one reading (生 on Okurigana, 人 on Kanji). Nothing about how other cards or terms are written changed. Readings were written for 々 (the iteration mark), Rendaku, Okurigana, Radical and its alternate forms, Kanji, Kun'yomi and on'yomi, Counter (the card and the definition, where 人 is にん as a counter), Pitch accent (the card and the definition), the particle reading card that closes the Particle page, and Keiyōshi. The Examples table on those pages reads each cell the same way, so 時 + 時 = 時々 has とき over each 時 and ときどき over 時々.

**4. Verb pairs.** The example sentence under each verb is `withFurigana` over its marked span, so お金が出た has かね and で and 出た is still the part picked out. The card said 67 pairs and 134 sentences. `transitivity.ts` has two: only 出る and 出す carry an example, on purpose (the field's own comment). Both are read.

**5. The sentence-ordering quiz.** Every sentence `assembly.ts` can deal (605 from the corpus plus the curated ones) is in the readings pass. `pieceSounds` in `src/data/sentence-readings.ts` cuts a sentence's readings where its pieces meet, and `orderCard` deals indices rather than strings so each piece keeps its readings: `QuizCard.order.sounds` is in the dealt order and `order.answerSound` is the whole sentence for the answer line in `quiz-verdict.tsx`. The grader still reads `pieces` and `answer`, which did not change.

**6. A kanji's origin.** `etymology` is a `SoundLine` now. Where the prose says "the sound of X" with no reading in brackets after it, X gets its on'yomi over it, the one closest to the kanji's own (主 on 住 is しゅう, not しゅ). 428 kanji gained a reading this way, 丁 over 可 among them. 430 more name a sound piece the kanji table has no on'yomi for (也 on 他, 弋 on 代), and those print as before. Phrasing other than "the sound of" ("可 for its sound") is left alone, and so is every piece named for its meaning.

**The readings pass.** `scripts/build-sentence-readings.ts` lists 832 sentences now, 713 of them new. `teach_sentence_readings.py` gained three things: a token with one kanji the aligner refuses (嬉しい, 凄い, 噂) is read from the token's own reading with its kana taken off; 何 before か, が, を, も or し is なに; and new token and sentence overrides where the tagger was wrong (開く said of a door is あく, 富士山 is ふじさん, 宇宙人 is うちゅうじん, 今日中 is きょうじゅう, 上手 is じょうず, and others, each with its reason in the file). Every new row was read by hand. No slot is null, and the 119 rows from before are unchanged.

**Tests.** `sound-line.test.ts` covers the run split, the prose builder and `rubyFromReading`. `teach.test.ts` checks that no slot in the readings file is null; that every particle page's prose has a reading over every kanji and still spells its text; 行くから and 本は in the Family table and no bare kanji in any Family cell; ときどき over 時々 and とき over each 時, 生 read い and う, 高 and 嫌 on Keiyōshi, 水 on Radical, 人 read にん in Counter's definition; that no term, writing-rule or concept page has a bare kanji other than the ones written null; that every authored reading lines up with its run and names a run the card has; 出る and 出す's sentences with かね, で and だ and the verb still picked out; 丁 over 可, しゅう over 主 on 住, and nothing added to 仕, whose reading is written in. `quiz.test.ts` checks the dealt ordering card's pieces carry their readings in the dealt order, 私は / 宿題を / 忘れてしまった。 piece by piece, and every sentence a card can deal with no kanji bare. The e2e tests open the 々 page and find ときどき over 時々, and open 出る and 出す and find かね, で and だ, with で inside the part picked out.

## The on'yomi of a sound part the kanji table does not have (SAK-486)

SAK-484 put a piece's on'yomi over it where a kanji's origin says "the sound of X", reading it from the kanji table. That table has only the 2,136 jōyō kanji, so a piece outside them (也 on 他, 弋 on 代, 丂 on 巧) printed plain.

**The table.** `scripts/ingest/sound-part-onyomi.ts` finds every piece an origin names for its sound that the kanji table has no on'yomi for, reads that piece's on'yomi from KANJIDIC2, and writes `src/data/generated/sound-part-onyomi.json`: piece to on'yomi in hiragana, in KANJIDIC2's order. 207 pieces are in it. `src/data/sound-part-onyomi.ts` loads it and holds `SOUND_OF`, the pattern that finds "the sound of X", which the ingest and `teach.ts` now share. `soundPartReading` in `teach.ts` reads the kanji table first and this table after it, and picks the on'yomi closest to the kanji's own the same way as before. 379 kanji gained a reading this way.

**Pieces past the basic plane.** The pattern used to match only the basic CJK blocks, so a piece like 𠂢 on 派 was never looked at. It now also matches the CJK extensions beyond the basic plane and the radical forms (⺕ on 雪), and the builder cuts the piece by its length, since such a piece is two UTF-16 units. KANJIDIC2 has on'yomi for a few of them (𠂢 は on 派, 𠂤 たい on 追).

**Still plain.** KANJIDIC2 2026-268 has no on'yomi for 34 pieces, named in 52 kanji: ⺕ 㐬 㕣 俞 关 兹 堇 复 夗 寉 开 睘 肙 隽 飠, and 19 past the basic plane (𠯑 on 話, 𢦏 on 載 and the rest). The ingest prints the list. Some are variant forms of a character KANJIDIC2 does have (俞 of 兪, 兹 of 茲, 开 of 幵); this card did not map variants, so they stay plain.

**The source.** KANJIDIC2 is downloaded from EDRDG to the ignored `scripts/ingest/raw/kanjidic-sound-parts/` and pinned in `sources.json` as `kanjidic2-sound-parts`, CC BY-SA 4.0. It is its own pin rather than the existing `kanjidic2` one: EDRDG publishes a new file every day and keeps no old ones, so the file that pin names can no longer be downloaded, and accepting a newer one under it would leave kanji.json, readings.json and radicals.json claiming a file they were not built from. Run it with `node --import ./src/lib/conjugate/test-hooks.mjs scripts/ingest/sound-part-onyomi.ts`, and add `--accept-source` to take a newer KANJIDIC2.

**Tests.** `teach.test.ts` checks や over 也 on 他, は over 𠂢 on 派, that both origins still spell their text, and that at most 34 pieces named for their sound are left with no reading across every kanji's origin. The e2e test opens 他 in the Atlas and finds や over 也.
## Readings on the Sessions list, and furigana in the Family footer (SAK-485)

**Sessions.** Under an open session, a word's row prints its reading beside the word, muted and smaller: 今日 きょう, 申し込み もうしこみ. The rule is the word chips' rule from SAK-483, `chipReading` in `src/sky/lib/japanese.ts`, so a kanji row and a word written in kana (これ) show no reading. The reading is `GlyphReading` in `src/sky/components/glyph.tsx`, a 13px muted span centered on the row beside `GlyphName`. The glyph column is 10rem on a wide screen and 9rem on a phone, up from 7rem and 5rem, which fits 申し込み もうしこみ at 400px wide. The English still truncates. If a word and its reading are longer than the column, the reading is cut first and the whole of it is in its tooltip. The row is 33px tall with or without a reading.

**The results after a quiz.** `quiz-results.tsx` lists the same kind of row, and it does the same thing with one difference: a card that asked for the reading already has the reading in its answer column (今日 きょう), so the reading beside the glyph is left off there instead of printing it twice.

**The pretend learner.** Every session the sample learner had was kanji mix-ups, so `/sessions?sample` had no word rows to show. `sample-learner.ts` now adds one newer run of seven words (今日, 申し込み, 食べる, これ, 図書館, お母さん, 大きい), each on its meaning card, with a mix of Perfect, With help and Missed. It is the newest session, so it is the one the page opens on.

**The Family footer.** The note under a Family table (the cluster's `feel` in `src/data/grammar/clusters.ts`) printed as plain text, so 東京から on 〜から had no reading. `TeachTable.note` can be runs now, and the family builder in `teach.ts` reads the note the way SAK-484 reads a particle page's prose: a word from the vocabulary, anything else from the readings pass. The note is drawn with `Sound` when it has readings and `Mixed` when it has none, at 14.5px, the size of the page's prose, because at the old 12.5px the kana over 東京 came out near 7px. `scripts/build-sentence-readings.ts` lists each Family note's runs for the pass, which added four rows, each checked by hand: 東京から (とう, きょう), 降りそう (ふ), 降るそうだ (ふ) and 食べられる (た). No other row changed.

**Tests.** `teach.test.ts` checks that 〜から's Family note has とう over 東 and きょう over 京 and still spells the note word for word, and that no Family note or footer has a kanji run without a reading. The e2e test opens `/sessions?sample` and finds もうしこみ on the 申し込み row and きょう on the 今日 row, これ printed once, and the 申し込み row the same height as the これ row.

## A shared particle note is three pages, and "が meaning but" (SAK-479, third round)

Sam, 2026-09-26, on the は card: "i think this should be named ~wa vs ~ga or something. i think we should split the pages up so page 1 is ~wa then has the section about wa then ~ga then the section about ga and then a page of when both are used. the family page also needs to be updated." And on the other pairs: に and で, まで and までに, だけ and しか, ね and よ shared one long page the same way, so they were split the same way.
...
## A meaning under each example for the words that matter (SAK-488)

Sam, 2026-09-26, on the が page's examples (雨が降っています, 水がほしいです, 誰が来ましたか): "the user might not know what the word means so maybe we should put a definition on the words that matter in all examples in all particles."

**The data.** Every `ParticleNoteExample` in `src/data/grammar/particle-notes.ts` has `words`, a list of `{ word, meaning, reading? }` in the order the sentence has them. `words` is required, so a new example without it does not compile. A verb or adjective is in its dictionary form (降る fall, 暑い hot), a noun as the sentence writes it (雨 rain, お金 money). Left out: the particle the example marks, the other particles, です, ます and だ, and a word the paragraph right above gives a meaning for in brackets or quotes. So 水がほしいです lists 水 and not ほしい, because the paragraph says "ほしい (want)". A word the prose only names without a meaning is still listed. 87 examples carry 172 words. One list is empty on purpose, 妹は歌が上手です, whose paragraph gives 妹, 歌 and 上手 their meanings, and a comment beside it says so.

**The meanings.** Each one is the vocabulary's first sense cut to a word or two (`SENSES` and `VocabRow.glosses` in `src/data/vocab.ts`). Where that first sense is not the one the sentence uses, the meaning is written by hand: いる (the first sense is 要る, "to be needed"), なる ("to bear fruit"), 五分 (ごぶ, "half"), 遅い ("slow", where the sentence means late) and 休み ("rest", where it means a day off). Words the vocabulary does not have are written by hand too: 田中さん, 東京, きれい, 勉強する and the clock times (七時, 二時, 六時, 九時, 三時間).

**The furigana.** `exampleWord` in `src/app/(sky)/teach.ts` gives each run of kanji in a word the reading the sentence gives the same run, so the word reads the way the line above it does (家 いえ, 今日 きょう as one run, 何 なん in これは何ですか). A dictionary form that reads its kanji another way carries `reading`: 来る is くる, where 来ました gives 来 the reading き. It is the only one.

**The page.** `TeachParagraph.examples` has `words`, each a `SoundLine` and a meaning. `ExampleWords` in `src/sky/components/teach-page.tsx` draws them under the English as one muted line, "雨 rain · 降る fall", at 12.5px with the Japanese at 14.5px, where the kana over it are about 8px. A word stays on one line with its meaning when the line wraps, and the line sits 4px below the English so the kana over its first word do not touch it.

**Tests.** `teach.test.ts`, "the words under each example": every example has words (the one empty list is pinned by name); each meaning is at most four words and has no dash; each word's stem is in its sentence, in order (its kanji, or its first two kana, or its first kana for a two-kana word like いる); no word is a particle the example marks, or です, ます or だ; and the words on each card are the words written, with a kana reading over every kanji, 家 いえ, 来る く and 日曜日 にちようび among them.
