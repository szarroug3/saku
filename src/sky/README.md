...
**What a tile is waiting on is now a prerequisite, not a sentence.** `ItemGate`, the one-line "what opens this" the card carried, is gone, and with it the `gates` map on a section and the dim look on `ItemCard`. In its place a sentence type takes the patterns it is still waiting on as its own parts (`waitingOn` in `src/app/(sky)/observatory.ts`), so the page's existing rule for what to lay out, `pickState(...).available`, is what hides it: every part learned or in tonight's cart, or no tile. Picking は and が brings "Simple" into the row, and taking either back out takes it away again, with the type's own pick removed by `withoutPick`, which has always dropped a pick that lost its footing.
