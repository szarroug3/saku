...
// destination by a rockier road": it is everCorrect, a monotonic OR over the
// round's retry legs (see RoundSummary and drill-stats.ts), so a round with a
// miss that got recovered before the round ended reads identically to a round
// with no miss at all. `missed` (facts that took at least one miss this
// round) is what actually distinguishes the two, and it is already on
// RoundSummary — so the comparison has to agree on BOTH counts before it can
// honestly call two rounds the same. See SAK-21: a miss in round 1, recovered
// within that same round, still had the recap claim nothing changed.
