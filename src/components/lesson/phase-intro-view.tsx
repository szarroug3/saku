  ...
  return (
    <div>
      ...
      {/* The body, off the same single light divider the item view uses, so a
          ...
          doesn't lurch between them. */}
      <div className="mt-8 space-y-4 border-t border-border pt-7">
        {intro.body.map((p, i) => (
          <div key={i} className="flex max-w-[64ch] items-start gap-3">
            {/* The mark the paragraph is about, at a size you can actually see
                it at. ゛ and ゜ are two specks in a run of body text, which is
                no way to introduce the thing the whole card is about. Given a
                fixed slot so the two paragraphs line up under each other and
                read as a pair of marks rather than a pair of sentences. */}
            {p.mark ? (
              <span
                aria-hidden
