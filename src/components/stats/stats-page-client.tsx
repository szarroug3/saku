...
// produce one.
//
// SAK-104: `totalFacts` (ALL_FACTS.length) is now a prop instead of a
// module-scope read of ALL_FACTS — see src/app/stats/page.tsx, the Server
// Component wrapper that computes it. ALL_FACTS itself lives behind
// data/vocab.ts's `server-only` guard; a Server Component can still import it
// directly (no client bundle involved), which is a plain prop rather than a
// Server Action round trip and so has no loading gap of its own. `factKeys`
// below comes from lib/fact-keys.ts instead of lib/facts.ts for the same
// reason — a pure Object.keys brand cast with no guarded dependency (see that
// file's own comment).
...
export function StatsPageClient({ totalFacts }: { totalFacts: number }) {
  ...
  return (
    ...
    <div className="max-w-4xl">
      ...
      <KnowledgeBase tally={tally} total={totalFacts} factsByBucket={factsByBucket} />
