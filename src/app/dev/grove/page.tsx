// Landing page for the Grove gallery: what this area is, and the rules that keep
// it clean. Route: /dev/grove

export default function GroveOverviewPage() {
  return (
    <div className="max-w-[72ch]">
      <Block title="Why it is separate">
        <p>
          The redesign replaces the Nursery, Garden, Quiz, Practice and Library
          ...
          own tree instead: <Code>src/grove/</Code>.
        </p>
      </Block>

      <Block title="The boundary, enforced by lint">
        <p>
          Grove code may not import from <Code>@/components</Code>,{" "}
          <Code>@/lib</Code> or <Code>@/data</Code>, and the existing app may not
          import from <Code>@/grove</Code>. Both directions are{" "}
        ...
        <p>
          If the Grove needs something the app already has, it gets its own copy.
      ...
      <Block title="The one thing that is shared: colour">
        <p>
          Grove components use the app&apos;s existing CSS tokens (
      ...
      <Block title="Cutover">
        <p>
          Point the real routes at Grove components, delete the old surface
