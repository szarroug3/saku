...
function QuizRun({ cards, run, source, back, sample, signedIn, rounds, cfg, update, router }: { cards: readonly QuizCard[]; run: SavedRun | null; source: RunSource; back: WayBack; sample: boolean; signedIn: boolean; rounds: number; cfg: QuizConfig; update: (patch: Partial<QuizConfig>) => void; router: ReturnType<typeof useRouter> }) {
  ...
  return <SkyQuiz key={`${deck}\n${round}`} cards={asked} grade={grade} toKana={typeKana} onFinish={finish} back={back} hear={HearButton} pitch={PitchMark} onRetry={retry} next={next} startAt={from ? resumeAt(from) : 0} startAnswers={from?.answers} onProgress={(state) => progress(state.at, state.answers)} retries={retriesOf(cfg)} onRetries={(n) => update(retriesPatch(n))} timerSeconds={cfg.timer ? cfg.timerSec : 0} height="100%" />;
}
