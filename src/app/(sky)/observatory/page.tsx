...
import { loadObservatory, loadPlace } from "../actions";
...
export default async function SkyObservatoryPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  // the page's own data and what was left part way through at the same time,
  ...
  return <ObservatoryClient sample={sample} signedIn={signedIn} initial={initial} picks={idsFrom(params.picks)} accountPlace={accountPlace} />;
}
