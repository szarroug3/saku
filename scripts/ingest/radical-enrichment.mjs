...
//   EquivalentUnifiedIdeograph.txt  Unicode UCD, permissive Unicode license.
//                             Normalizes a radical-block glyph (Kangxi block
...
//   - A variant whose normalized glyph EQUALS its base glyph (⺌ しょうかんむり → 小;
//     おおざと → 邑). Same shape as the base under Unicode's equivalence; not a
//     distinct form to carry.
//   - A variant whose normalized glyph is ANOTHER radical's base glyph. Kanji
...
// every variant glyph is a CSV glyph normalized by the Unicode equivalence
...
// Normalize a radical-block glyph to the CJK unified ideograph it is a form of.
...
function main() {
  ...
  const HEADER =
    ...
    "CSV's radical-block codepoints normalized to their CJK unified ideograph " +
